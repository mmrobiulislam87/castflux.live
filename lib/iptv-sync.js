...
async function fetchJSON(url) {
  const res = await fetch(url, {
    headers: { 'User-Agent': 'LiveTV-Local/1.0' },
...
async function syncIptvOrg(options = {}) {
  const { country, limit = 250, categories = [] } = options;
  ...
  for (const stream of streams) {
    ...
    if (categories.length && ch && !categories.some((c) => ch.categories?.includes(c))) continue;
...
async function syncFreeTv(options = {}) {
...
module.exports = { syncIptvOrg, syncFreeTv, IPTV_ORG, FREE_TV };
